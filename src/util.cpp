...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NanoCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NanoCoin
    // Mac: ~/Library/Application Support/NanoCoin
    // Unix: ~/.NanoCoin
#ifdef WIN32
    // Windows
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("NanoCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
