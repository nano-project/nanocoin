...
{
    if(nHeight == 1) return 1111 * COIN;
    if(nHeight >= 2 && nHeight <= 1111) return 666  * COIN;
    if(nHeight >= 1112 && nHeight <= 2222) return 599.999 * COIN;
    if(nHeight >= 2223 && nHeight <= 3333) return 555.555 * COIN;
    if(nHeight >= 3334 && nHeight <= 4444) return 545.545 * COIN;
    if(nHeight >= 4445 && nHeight <= 5555) return 544.445 * COIN;
    if(nHeight >= 5556 && nHeight <= 6666) return 535.535 * COIN;
    if(nHeight >= 6667 && nHeight <= 7777) return 533.533 * COIN;
    if(nHeight >= 7778 && nHeight <= 8888) return 531.531 * COIN;
    if(nHeight >= 8889 && nHeight <= 9999) return 525.521 * COIN;
    if(nHeight >= 10000 && nHeight <= 11111) return 499.499 * COIN;
    if(nHeight >= 11111 && nHeight <= 13131) return 99 * COIN;
    ...
    // NanoCoin: subsidy is cut in half every 64x multiply of PoW difficulty
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("NanoCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in NanoCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // NanoCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NanoCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "NanoCoin(NC)";
        unsigned int nTimeGenesis = 1514267181;
        unsigned int nNonceGenesis = 486922;

        if (fTestNet)
        {
            pszTimestamp ="NanoCoin(NC)";
        ...
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xe948c76a827dfe66fc1b6ebd401378595aecb4cf46f4001666643d7b4a3e7954"));
        else
...
unsigned char pchMessageStart[4] = { 0x65, 0xdb, 0x8b, 0xae };
