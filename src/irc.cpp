...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NanoCoinTEST\r");
            Send(hSocket, "WHO #NanoCoinTEST\r");
        } else {
            // randomly join #NanoCoin00-#NanoCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 1 for initial release
            int channel_number = 1;
            Send(hSocket, strprintf("JOIN #NanoCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NanoCoin%02d\r", channel_number).c_str());
        }
